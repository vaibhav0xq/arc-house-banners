...
export function ExplodedBanner({ country }: { country: Country }) {
  ...
  const spread = pressed ? 0 : 1;
  const z = (base: number) => base * spread;
  ...
  return (
    ...
    >
      ...
      >
        {/* 01 scene */}
        <motion.div
          className={`${sheet} overflow-hidden bg-white shadow-[0_34px_60px_-22px_color-mix(in_srgb,var(--ink)_50%,transparent)]`}
          animate={{ z: z(0) }}
        ...
        <motion.div
          ...
          animate={{ z: z(90) }}
          transition={{ type: 'spring', stiffness: 120, damping: 18, delay: 0.03 }}
        >
          <PapercutLockup country={country} city={country.defaultCity} handle="yourhandle" placeholder={false} className="w-full h-full" />
        </motion.div>

        {/* 03 portrait */}
        <motion.div className={`${sheet} border-transparent`} animate={{ z: z(180) }} transition={{ type: 'spring', stiffness: 120, damping: 18, delay: 0.06 }}>
